  ...
  for (const [categoryName, items] of Object.entries(catalog)) {
    ...
    for (const [name, price] of items) {
      ...
      await prisma.product.upsert({
        where: { sku },
        update: {},
        create: { name, sku, categoryId: category.id, sellingPrice: price ?? 0, costPrice: 0, stockQuantity: 0, minimumStock: 0, status: 'ACTIVE', localId: `catalog-${slug(name).toLowerCase()}` },
      });
      imported += 1;
    }
  }
