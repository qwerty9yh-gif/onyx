...
export const CustomersPage: React.FC = () => {
  ...
  const broadcastMutation = useMutation({
    ...
    onSuccess: (res) => setFeedback(res.success
      ? `Broadcast sent to ${res.sent} customer${res.sent === 1 ? '' : 's'}.`
      : `Broadcast could not be completed: ${res.sent} sent, ${res.failed} failed.`),
    onError: (err) => setFeedback(handleApiError(err)),
  });
    ...
    <div className="space-y-5">
      ...
      <Modal open={Boolean(messageTarget)} onClose={() => setMessageTarget(null)} title={`SMS to ${messageTarget?.name || ''}`} size="md">
        <div className="space-y-4">
          ...
          {feedback && <p className={`text-sm font-medium ${feedback.includes('could not') ? 'text-red-600' : 'text-emerald-700'}`}>{feedback}</p>}
          <div className="flex justify-end gap-2">
