...
export const TransactionsPage: React.FC = () => {
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    <header><p className="text-sm font-semibold uppercase tracking-widest text-brand-700">Transactions</p><h1 className="text-3xl font-bold">Sales history</h1></header>
    <div className="flex flex-col gap-4 rounded-3xl border border-red-100 bg-white/85 p-4 shadow-lg shadow-red-950/10 sm:flex-row"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-brand-700" size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search receipt or customer" className="h-12 w-full rounded-2xl bg-red-50 pl-10 pr-4 outline-none focus:ring-2 focus:ring-red-300" /></div><div className="flex rounded-2xl bg-red-50 p-1" role="tablist" aria-label="Transaction status"><button type="button" role="tab" aria-selected={tab === 'paid'} onClick={() => setTab('paid')} className={`flex-1 rounded-xl px-5 py-3 text-sm font-bold transition ${tab === 'paid' ? 'onyx-brand-gradient text-white shadow-glow-red' : 'text-brand-700 hover:bg-white'}`}>Paid</button><button type="button" role="tab" aria-selected={tab === 'unpaid'} onClick={() => setTab('unpaid')} className={`flex-1 rounded-xl px-5 py-3 text-sm font-bold transition ${tab === 'unpaid' ? 'onyx-brand-gradient text-white shadow-glow-red' : 'text-brand-700 hover:bg-white'}`}>Unpaid</button></div></div>
    ...
    {!isLoading && !isError && section(tab === 'paid' ? 'Paid transactions' : 'Unpaid transactions', tab === 'paid' ? paid : unpaid)}
