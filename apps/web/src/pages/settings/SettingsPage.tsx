...
import { Download, Edit3, Plus, Save, Search, Trash2, Upload } from 'lucide-react';
...
export const SettingsPage: React.FC = () => {
  ...
  return <div className="mx-auto max-w-7xl space-y-6"><header><p className="text-sm font-semibold uppercase tracking-widest text-sky-700">Settings</p><h1 className="text-3xl font-bold">Management center</h1></header><section className="grid gap-5 lg:grid-cols-2"><div className="rounded-3xl border border-white/80 bg-white/85 p-6 shadow-xl shadow-slate-200/50"><h2 className="text-xl font-bold">General</h2><div className="mt-4 grid gap-4 sm:grid-cols-2"><Input label="Store name" value={form.name} onChange={(event) => setBusiness({ name: event.target.value })} /><Input label="Currency" value={form.currency} onChange={(event) => setBusiness({ currency: event.target.value })} /><Input label="Phone" value={form.phone} onChange={(event) => setBusiness({ phone: event.target.value })} /><Input label="Email" value={form.email} onChange={(event) => setBusiness({ email: event.target.value })} /><div className="sm:col-span-2"><Input label="Address" value={form.address} onChange={(event) => setBusiness({ address: event.target.value })} /></div></div><Button className="mt-5 rounded-2xl bg-sky-600" onClick={() => saveSettings.mutate()} loading={saveSettings.isPending}>Save general settings</Button></div><div className="rounded-3xl border border-white/80 bg-white/85 p-6 shadow-xl shadow-slate-200/50"><h2 className="text-xl font-bold">Security & backup</h2><p className="mt-2 text-sm text-slate-500">Admin-only controls for users, roles, audit logs, and local data backup.</p><div className="mt-5 flex flex-wrap gap-3"><Button variant="outline" className="rounded-2xl" onClick={() => window.location.assign('/users')}>User management</Button><Button variant="outline" className="rounded-2xl" onClick={exportProducts}><Download className="mr-2" size={16} />Backup products</Button></div></div></section><section className="rounded-3xl border border-white/80 bg-white/85 p-6 shadow-xl shadow-slate-200/50"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold text-sky-700">Products</p><h2 className="text-2xl font-bold">Product management</h2></div><div className="flex flex-wrap gap-2"><Button className="rounded-2xl bg-sky-600" onClick={() => window.location.assign('/products/new')}><Plus className="mr-2" size={16} />Add product</Button><Button variant="outline" className="rounded-2xl" onClick={exportProducts}><Download size={16} /></Button><Button variant="outline" className="rounded-2xl" onClick={() => fileInput.current?.click()}><Upload size={16} /></Button><input ref={fileInput} type="file" accept="application/json" hidden onChange={importProducts} /></div></div><div className="mt-5 grid gap-3 md:grid-cols-[1fr_180px_180px_180px]"><div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-sky-600" size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search products" className="h-12 w-full rounded-2xl border-0 bg-slate-100 pl-10 pr-3 outline-none focus:ring-2 focus:ring-sky-300" /></div><select value={categoryId} onChange={(event) => setCategoryId(event.target.value)} className="h-12 rounded-2xl border-0 bg-slate-100 px-3"><option value="">All categories</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select><select value={sort} onChange={(event) => setSort(event.target.value as SortMode)} className="h-12 rounded-2xl border-0 bg-slate-100 px-3"><option value="name">Sort: Name</option><option value="price">Sort: Price</option><option value="category">Sort: Category</option></select><div className="flex gap-2"><input value={bulkPercent} onChange={(event) => setBulkPercent(event.target.value)} placeholder="% change" type="number" className="h-12 min-w-0 w-full rounded-2xl border-0 bg-slate-100 px-3" /><button type="button" onClick={applyBulkPrice} className="rounded-2xl bg-slate-900 px-3 text-white" title="Apply bulk price change">%</button></div></div><div className="mt-5 space-y-3">{visibleProducts.map((product) => { const isEditing = editing === product.id; return <div key={product.id} className="grid gap-3 rounded-2xl bg-sky-50/70 p-4 md:grid-cols-[1fr_150px_130px_180px_auto] md:items-center"><div><p className="font-bold">{product.name}</p><p className="text-xs text-slate-500">{product.sku}</p></div>{isEditing ? <input type="number" step="0.01" value={Number(draft.sellingPrice ?? product.sellingPrice)} onChange={(event) => setDraft({ ...draft, sellingPrice: Number(event.target.value) })} className="h-10 rounded-xl border-0 bg-white px-3" /> : <strong className="text-sky-700">GH₵{product.sellingPrice.toFixed(2)}</strong>}{isEditing ? <input type="number" value={Number(draft.stockQuantity ?? product.stockQuantity)} onChange={(event) => setDraft({ ...draft, stockQuantity: Number(event.target.value) })} className="h-10 rounded-xl border-0 bg-white px-3" /> : <span className="text-sm">Stock: {product.stockQuantity}</span>}{isEditing ? <select value={String(draft.categoryId ?? product.categoryId ?? '')} onChange={(event) => setDraft({ ...draft, categoryId: event.target.value || null })} className="h-10 rounded-xl border-0 bg-white px-3"><option value="">Uncategorized</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select> : <span className="text-sm text-slate-500">{product.category?.name || 'Uncategorized'}</span>}<div className="flex justify-end gap-2">{isEditing ? <button type="button" onClick={() => updateProduct.mutate({ id: product.id, data: draft })} className="rounded-xl bg-emerald-600 p-2 text-white" title="Save"><Save size={16} /></button> : <button type="button" onClick={() => { setEditing(product.id); setDraft(product); }} className="rounded-xl bg-white p-2 text-sky-700" title="Edit"><Edit3 size={16} /></button>}<button type="button" onClick={() => window.confirm(`Delete ${product.name}?`) && deleteProduct.mutate(product.id)} className="rounded-xl bg-white p-2 text-red-600" title="Delete"><Trash2 size={16} /></button></div></div>; })}</div>{saveSettings.isError && <p className="mt-3 text-sm text-red-600">{handleApiError(saveSettings.error)}</p>}</section></div>;
};
