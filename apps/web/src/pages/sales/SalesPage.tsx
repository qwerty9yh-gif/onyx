...
export const SalesPage: React.FC = () => {
  ...
  const [smsStatus, setSmsStatus] = useState<'' | 'sending' | 'done' | 'error'>('');
  ...
  const markPaidMutation = useMutation({
    ...
    onSuccess: (res) => {
      ...
      if (!receiptNumber.startsWith('OFF-')) {
        ...
        setSmsContactPhone(customerPhone.trim());
      }
      clearCart();
      setCart([]);
      setAmountReceived('');
...
const sendSmsReceipt = async (saleId: string, phone: string) => {
    ...
    try {
      const result = await sendSmsInvoice(saleId, phone);
      setSmsStatus(result.success ? 'done' : 'error');
      setToast(result.success ? 'Receipt sent via SMS' : (result.error || 'SMS could not be sent'));
    } catch {
      setSmsStatus('error');
      setToast('SMS could not be sent');
    }
    window.setTimeout(() => setToast(''), 3000);
  };
        ...
        <section className="space-y-4">
          ...
          <div className="grid h-[18.5rem] max-h-[18.5rem] grid-cols-2 auto-rows-[8.5rem] gap-3 overflow-y-auto rounded-3xl border-2 border-brand-200 bg-red-50/50 p-3 pr-2 shadow-xl shadow-red-950/10">
            ...
            {!productsLoading && products.map((product) => (
              <button type="button" key={product.id} onClick={() => addToCart(product)} disabled={product.stockQuantity <= 0} className="group rounded-2xl border border-red-200 bg-gradient-to-br from-red-700 via-red-600 to-red-500 p-3 text-left shadow-lg shadow-red-950/20 transition hover:-translate-y-1 hover:border-red-100 hover:shadow-glow-red disabled:cursor-not-allowed disabled:opacity-50">
                <span className="mb-2 flex h-10 w-10 items-center justify-center rounded-2xl bg-white/20 text-white"><ShoppingCart size={20} /></span>
                <span className="block truncate font-bold text-white drop-shadow-sm">{product.name}</span>
                <span className="mt-1 block text-sm text-white/80">{money(product.sellingPrice)} · {product.stockQuantity} in stock</span>
              </button>
            ))}
...
<aside ref={cartRef} className="rounded-3xl border border-white/80 bg-white/85 p-5 shadow-2xl shadow-slate-300/40 backdrop-blur-xl">
          ...
          <div className="space-y-3 rounded-2xl border border-red-100 bg-red-50/50 p-3">
            ...
            <label className="block">
              ...
              <input type="tel" value={customerPhone} onChange={(event) => setCustomerPhone(event.target.value)} placeholder="e.g. 0551234567" className="mt-1 h-11 w-full rounded-xl border border-red-100 bg-white px-3 text-sm outline-none transition focus:border-brand-400 focus:ring-2 focus:ring-red-300" />
            </label>
          </div>
...
{printPrompt && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4" role="dialog" aria-modal="true" aria-labelledby="print-receipt-title">
          <div className="w-full max-w-sm rounded-3xl bg-white p-6 shadow-2xl">
            <h2 id="print-receipt-title" className="text-xl font-bold text-slate-900">Sale completed</h2>
            <p className="mt-2 text-sm text-slate-500">Payment saved for {printPrompt.receiptNumber}.</p>
            {receiptViewId && smsContactPhone && (
              <div className="mt-3 flex items-center gap-2 rounded-2xl bg-red-50 p-3 text-sm text-brand-800">
                <Phone size={16} />
                <span>SMS receipt to <strong>{smsContactPhone}</strong></span>
              </div>
            )}
            <div className="mt-6 grid grid-cols-2 gap-3">
              ...
              {receiptViewId && smsContactPhone && (
                <Button className="rounded-2xl bg-brand-700 text-white hover:bg-brand-800" variant="primary" loading={smsStatus === 'sending'} onClick={() => sendSmsReceipt(receiptViewId, smsContactPhone)}>
                  <Send className="mr-2" size={18} />Send receipt by SMS
                </Button>
              )}
